...
with tabs[8]:
    ...
    if curve_is_sample(curve_snapshot):
        ...
        st.download_button("⬇️ Télécharger l’export Excel complet", data=excel_bytes(sheets), file_name="valorisation_obligataire_maroc_v15_1.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        st.download_button("⬇️ Télécharger l’export simplifié Excel", data=excel_bytes(simplified_sheets), file_name="export_simplifie_valorisation_v15_1.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        try:
            ...
            st.download_button("📄 Télécharger l’export simplifié PDF", data=pdf_simple, file_name="export_simplifie_valorisation_v15_1.pdf", mime="application/pdf", use_container_width=True)
        except Exception as e:
    ...
    if st.button("📄 Générer le rapport PDF détaillé"):
        try:
            ...
            st.download_button("Télécharger le rapport PDF détaillé", data=pdf, file_name="rapport_valorisation_obligataire_v15_1.pdf", mime="application/pdf")
        except Exception as e:
            st.error(str(e))
